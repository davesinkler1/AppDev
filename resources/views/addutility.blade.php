...
@section('content')
        <div class="hidden fixed top-0 left-0 px-0 py-0 sm:block">
            <a href="{{ url('/home') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Back to Home</a>
        </div>
        <div class="">
        @if(session('status'))
        ...
        <form class="text-center " action='/home/storeutility' method="POST" enctype="multipart/form-data">
            @csrf
            <!-- Car Brand -->
            ...
            <!-- Upload Image -->
            <div class="mt-4">
                <x-label for="img" :value="__('Upload Utility Image')" />
                <x-input type="file" id="img" name="image" class="block mt-1 w-full"/>
            </div>
